...
SELECT b.Author AS '저자' -- 1 DATABASE는 1부터
  -- , b.Division AS '장르' -- 2
     , (SELECT Names 
          FROM divtbl 
     ...
     , b.Price AS '금액'
  FROM bookstbl AS b
 ORDER BY b.Names ASC; -- 책제목, 3 => b.Nams 대신해서 쓸 수 있는 방법
...
SELECT m.Names AS '이름'
     ...
     , IFNULL(r.renturnDate, '미반납') AS '반납일자'
     , b.Names AS '책제목'
     , b.Names AS '장르'
     , b.Price AS '금액'
  FROM membertbl AS m
...
-- 책을 두권이상 출판한 저자
SELECT bb.Author, bb.Division, bb.출판권수, bb.합계금액
  FROM (
	    SELECT b.Author
			 , b.Division
